...
export class EstudianteDatosAdicionalesComponent implements OnInit {
  ...
  construirGrupos(catalogo: any[], valores: any[], soloConValor: boolean): GrupoVista[] {
    ...
    valores.forEach((v: any) => {
      valoresMap.set(v.id_dato_adicional, v);
    });

    const gruposMap = new Map<string, GrupoVista>();

    catalogo.forEach((item: any) => {
      const idTipo = item.id_tipo_dato_adicional;
  ...
  activarEdicion(): void {
    ...
    this.datosAdicionalesService.obtenerTodos().subscribe({
      ...
      error: () => {
        this.cargando = false;
        Swal.fire('Error', 'No se pudo cargar el catálogo de datos adicionales', 'error');
      }
    });
  }
  ...
  guardarDatos(): void {
    ...
    this.datosAdicionalesXEstudianteService.guardarPorEstudiante(this.idEstudiante, datos).subscribe({
      next: () => {
        ...
        Swal.fire({
          ...
          text: 'Los datos adicionales han sido actualizados',
      ...
      error: () => {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'No se pudieron actualizar los datos adicionales',
          confirmButtonColor: '#FFC107',
        });
      }
    });
